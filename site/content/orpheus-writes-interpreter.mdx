...
`execute()` will call `evaluate()` for _evaluating_ basic expressions, but `execute()` will be doing the. It's similar to the recursive descent we applied while writing our parser, except now we've condensed it down to three methods - `run()` for blocks of statements, `execute()` for those statements, and `evaluate()` for expressions.
