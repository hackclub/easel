...
<aside> 
  This page is more in beta than the other pages... especially plot wise. Working on it!
</aside>
...
<aside>
  Is that... [Conway's Game of Life](https://playgameoflife.com/)? Yup, and we're going to get it to run on an easel.
</aside>
...
sketch getNeighbors needs (cells, index) {
  ...
  if (index + rows + 1 < cells.length - 1) {
    neighbors.ass(cells[index + rows + 1])
  }
