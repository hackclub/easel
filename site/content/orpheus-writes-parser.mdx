...
<aside>
  We technically don't need the `type` property, but we'll include it because it's useful debug info.
</aside>
...
<aside>
  This is pretty repetitive, ain't it? There are [parser generators](https://web.mit.edu/6.005/www/fa15/classes/18-parser-generators/) and such to deal with this out there (especially once things get more complicated), but buckle up, because we're writing this from scratch!
</aside>
...
Let's test out everything we have so far. Let's add our parser to `easel.js`:
...
Try running `node easel.js test.easel --dbg`, and you'll get a new debug file containing the AST in `ast.json`. Mine looks something like this:
