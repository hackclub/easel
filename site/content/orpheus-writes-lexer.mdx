...
<aside>
  Narrator: today I'm craving a smoothie, not tea. You can also have a smoothie. Just saying.
</aside>
...
<aside>
  ...
  quite easy. It just involves a lot of *rawr*s of different intensities. Hey hey! Potential programming language?
</aside>
...
Cool! Now we can see what the next character is. The most logical step, now that we can peek at the next character, is to actually move to the next character. Let's call this advancing. It'll look pretty similar to `peek()`, except we'll update what character we're on rather than just looking ahead:
...
Run `node easel.js test.easel --dbg` and you'll get this in `tokens.json`:
...
_The complete code here is at [lexer.js](https://github.com/hackclub/easel/blob/main/languages/easel/lexer.js)._
